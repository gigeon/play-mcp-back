...
public class RecommendHousingTool implements McpTool {
    ...
    @Tool(description = """
        ...
        [안내] 정확한 추천을 위해 아래 6가지 정보가 필요해요. 사용자가 말하지 않은 값은 추측하지 말고,
        빠진 정보가 있으면 먼저 상냥하게 여쭤본 뒤에 추천해 주세요. 이렇게 물어보면 좋아요:
          - region: "어느 지역에서 찾고 계세요? 예: 서울 강남구처럼 구까지 알려주시면 더 정확해요 :)"
          - housingType: "아파트, 오피스텔, 빌라 중 어떤 집을 원하세요?"
          - dealType: "전세, 월세, 매매 중 어떤 방식으로 알아보고 계세요?"
          - age: "나이가 어떻게 되세요?"
          - married: "미혼이신가요, 기혼이신가요?"
          - income: "연소득이 대략 얼마나 되세요? 만원 단위로 편하게 알려주세요."
        여섯 가지가 모두 모이면 그때 딱 맞는 곳을 찾아드릴게요.
